...
uint256 hashGenesisBlock("0x00000790feb043ffdc20d62c9b21b39f0900afcff8a7d21c00058ef7c7c16fcd"); //mainnet
...
{
    // 42: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // 42: 1 day
static const int64 nTargetSpacing = 3 * 60; // 42: 2.5 minutes
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, 42 - DarkGravity v2, written by Evan Duffield - evan@42.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
// For 42 also add the protocol rule that the first output in the coinbase must go to the charity address and have at least 2.5% of the subsidy (as per integer arithmetic) (thx to einsteinium!)
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // 42: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // 42: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "Wired 10/Apr/2014 Has the NSA Been Using the Heartbleed Bug as an Internet Peephole?";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1397139617;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 32203441;
...
/**/
        ...
        assert(block.hashMerkleRoot == uint256("0xd8a429a789343ac2850914f70e49244e004504c4f4cc2458eedf956e647f88e3"));
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb7, 0xde }; // 42: increase each by adding 2 to bitcoin's value.
...
//    printf("%d\n", scriptPubKeyIn[0]);
    ...
    int64 nFees = 0;
    {
		...
		// With 42, at least 2.5% of all of the block subsidy should go to the charity address. (thx to einsteinium)
